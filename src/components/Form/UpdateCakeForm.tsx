...
const UpdateCakeForm = ({
  ...
  cake: any;
  handleCakeUpdate: (data: any) => Promise<void>;
  loading: boolean;
  closeModal: () => void;
}) => {
  const { image, name, category, price, quantity, description } = cake || {};
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Validate
      if (!cakeData.name.trim() || !cakeData.category.trim()) {
        toast.error("Name and category are required");
        return;
      }

      if (isNaN(cakeData.price) || cakeData.price <= 0) {
        toast.error("Price must be a positive number");
        return;
      }

      if (isNaN(cakeData.quantity) || cakeData.quantity < 0) {
        toast.error("Quantity must be 0 or more");
        return;
      }

      await handleCakeUpdate(cakeData);
      closeModal();
    } catch (error) {
      ...
      <form className="w-full" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="space-y-2">
            ...
            <textarea
              id="description"
              placeholder="Write cake description here..."
              ...
              <div className="space-y-2 pl-6">
                ...
                <div className="mt-1 relative rounded-md shadow-sm">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    required
                    defaultValue={cake?.discount?.discountPrice}
                  />
                </div>
              </div>
            )}
