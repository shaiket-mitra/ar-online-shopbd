...
export default function AllDiscountedProductsPage() {
  const { data: discountedProducts = [], isLoading } = useQuery({
    queryKey: ["discountedProducts"],
    queryFn: async () => {
      const { data } = await axios.get("/api/get-all-discountedProducts");
      return data;
    },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      >
        ...
        >
          ...
          <span className="font-medium">
            {discountedProducts.length} Special Offers
          </span>
        </motion.div>
      ...
      {discountedProducts && discountedProducts.length > 0 ? (
        ...
        >
          {discountedProducts.map((product: any, index: number) => (
            <motion.div
              key={product._id}
              initial={{ opacity: 0, y: 20 }}
      ...
      >
        <div className="inline-flex items-center bg-pink-100 text-pink-600 px-4 py-2 rounded-full text-sm font-medium">
          <FiTag className="mr-2" />
          Showing all {discountedProducts.length} discounted items
        </div>
      </motion.div>
    </div>
  );
