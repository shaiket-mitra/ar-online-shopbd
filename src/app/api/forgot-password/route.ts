...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const mailInfo = {
            to: email,
            subject: "Account Recovery - OTP Code",
            text: `Hello ${existingUser.name},

            Thank you for stay with Mitra Cake Shop!

            To complete your Recovery, please use the following One-Time Password (OTP):
        ...
        const result = await usersCollection.updateOne(
            { email },
            { $set: { forgetPasswordOtp: otp, otpExpiry: expiry } }
        );

        if (result.modifiedCount > 0 && response.success) {
        ...
        console.log("Problem in forgot password route", error)
